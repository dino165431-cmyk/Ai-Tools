...
function normalizeUsage(raw = {}) {
  const inputTokens = finiteNonNegative(raw.inputTokens ?? raw.input_tokens ?? raw.prompt_tokens)
  const outputTokens = finiteNonNegative(raw.outputTokens ?? raw.output_tokens ?? raw.completion_tokens)
  const cachedTokens = finiteNonNegative(
    raw.cachedTokens ?? raw.cached_tokens ??
    raw.input_tokens_details?.cached_tokens ?? raw.prompt_tokens_details?.cached_tokens
  )
  const totalTokens = finiteNonNegative(raw.totalTokens ?? raw.total_tokens) || inputTokens + outputTokens
...
async function recordUsage(entry = {}) {
  ...
  if (!usage.totalTokens && !usage.inputTokens && !usage.outputTokens) return { recorded: false }
  const now = new Date()
